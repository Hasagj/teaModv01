...
public class FinishUseEvent {
    ...
    public void onItemUseFinish(LivingEntityUseItemEvent.Finish event) {
        ...
        if (entity.hasEffect(ModEffects.BITTER_EFFECT) && entity.level() instanceof ServerLevel level) {
            if (entity instanceof Player player) {
                player.getFoodData().eat((int)(Objects.requireNonNull(usedItem.getItem().components().get(DataComponents.FOOD)).nutrition() * -0.75) - 1, (int) (Objects.requireNonNull(usedItem.getItem().components().get(DataComponents.FOOD)).saturation() * -0.75) - 1);
